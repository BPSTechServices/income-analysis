...
options(
  ...
  stringsAsFactors = F, # string variables are brought in as characters
  dplyr.width = Inf,
...
## Function to more easily convert state-county integers to padded characters
stcnty_int2char <- function(.x, stfips = "stfips", cntyfips = "cntyfips", newvar = "stcnty_fips"){
  .x %>%
    mutate("{newvar}" := paste0(str_pad(!!sym(stfips), 2, pad = "0"), str_pad(!!sym(cntyfips), 3, pad = "0")))
}
...
## In "age" and "pov_code," W stands for working age, and R stands for retired (65+)
...
hud_ami <- read_csv("data/resources/limits_2000t2020.csv", col_types = "cddcccdd") %>% 
  janitor::clean_names() %>% 
  stcnty_int2char(newvar = "stcnty_fips") %>%
  select(stcnty_fips, stfips, cntyfips, msafips, msa_name, hamfi = mfi, year) %>% 
  arrange(desc(year), stcnty_fips) %>%
  distinct(stcnty_fips, hamfi, year, .keep_all = T)

puma_county_xwalk12 <- "data/resources/geocorr_puma2010_county2010.csv" %>% 
...
puma_county_xwalk <- rbind(puma_county_xwalk12, puma_county_xwalk00) %>% 
  arrange(desc(pumayear), state, puma, desc(afact)) %>% 
  distinct(state, puma, .keep_all = TRUE) %>%
  stcnty_int2char(.x = ., stfips = "state", cntyfips = "county")
...
pums_raw <- read_ipums_micro(read_ipums_ddi("data/raw/usa_00120.xml")) %>%
  ...
  mutate_at(vars(statefip, puma, year:cluster, strata, owncost:hhincome, rooms, pernum, age, yrnatur, yrimmig, inctot:incearn), as.numeric) %>% ## TODO double check these
  mutate_at(vars(occ:indnaics), as.character) 
